...
## 1️⃣ Full North America OSRM Attempt

**Goal**  
Download the full North America extract from [Geofabrik](https://download.geofabrik.de/) and run a local **OSRM** server with a custom `truck.lua` profile for realistic heavy-vehicle routing.
...
OSRM extraction and partitioning exhausted 16 GB RAM and swap space; preprocessing never completed.
Docker container eventually terminated due to memory pressure.

2️⃣ Full North America with Valhalla
Goal
...
3️⃣ Regional Split: East / Central / West OSRM Servers
Goal
...
4️⃣ Osmium-Filtered Highways Only
Goal
...
Partial extractions were successful but the routing server failed to start due to incomplete graph coverage.

5️⃣ Smaller Subsets & Fewer Routes
Goal
...
6️⃣ Current Approach – OpenRouteService (ORS) API
...
✅ Result
Reliable, reproducible, and lightweight.
...
🧾 Final Note
The OpenRouteService API workflow (ors_geopandas_pipeline.py) proved to be the most practical and efficient approach given the hardware constraints.
All subsequent analytics — distance histograms, duration estimation, and minute-level simulations — are based on the ORS-generated routes.
